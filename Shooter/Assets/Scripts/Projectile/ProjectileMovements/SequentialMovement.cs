...
public class SequentialMovement : ProjectileMovement
{
    ...
    protected override void Awake()
    {
        ...
        /*ProjectileMovements = GetComponentsInChildren<ProjectileMovement>();

        if (ProjectileMovements.Length <= 0)
        {
            throw new Exception("A projectile with the SequentialMovement component must have a child GameObject attached with ProjectileMovements to run.");
        }

        if (StartTimes.Length != ProjectileMovements.Length - 1)
        {
            throw new Exception("Number of StartTimes must be 1 less than the number of ProjectileMovements in the child.");
        }*/
    }
